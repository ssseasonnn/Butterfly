...
interface InnerDispatcher {
    suspend fun dispatch(request: AgileRequest): Flow<Result<Bundle>> = emptyFlow()

    suspend fun dispatchByActivity(activity: FragmentActivity, request: AgileRequest): Flow<Result<Bundle>> = emptyFlow()
